...
my $script = Bio::KBase::AppService::AppScript->new(\&process_viral_assembly, \&preflight);
...
sub process_viral_assembly
{
    my($app, $app_def, $raw_params, $params) = @_;

    print 'Proc viral assembly ', Dumper($app_def, $raw_params, $params);
    ...
    my @cmd = ("run_viral_assembly","-o",$work_dir,"--jfile", $jdesc);
